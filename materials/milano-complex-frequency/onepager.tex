...
For \texttt{effective\_inertia}: the deck's TSC (port-Hamiltonian) separates stored energy
$H_k$ (Cond.\,1) from the dynamic slack to sustain $\Delta P$ (Cond.\,2) --- structurally the
same storage-vs-delivery split as your $H_{\rm load}$ vs $H_{\rm eff}$ / $r$ (that framing is
yours). $\cb\eta=\rho+j\omega$: you measure the $\omega$ (RoCoF) side --- computing $\rho=\dot
v/v$ too (you already log $v_{\rm rms}$) could attribute your $\sim$0.2\,s offset to voltage
vs frequency; $\rho$ is also your CMLD voltage-response / feeder question. Low-inertia/IBR
displacement + Dual-GFM motivate the setting.
\end{multicols}
\end{document}
